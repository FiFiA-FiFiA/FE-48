...
async function Get__AllUsers__Data(All__Users) {
  ...
  function Follow__User() {
    ...
    if (!All__User__Arr[i].FollowerUser.includes(Ovner__User)) {
      let FollowerUser = All__User__Arr[i].FollowerUser;
      let Temp__FollowerUser = [];

      for (const Follower of FollowerUser) {
        Temp__FollowerUser.push(Follower);
      }
      Temp__FollowerUser.push(Ovner__User);

      FBW.Update__User__FollowInfo__ById(All__User__Arr[i].Id, Temp__FollowerUser, []);
      Follow__Btn__Wrapper.classList.replace('Follow', "Unfollow");
    } else {
      Follow__Btn__Wrapper.classList.replace('Unfollow', "Follow");
    }
    ...
    if (!Ovner__User.FollowingUser.includes(All__User__Arr[i])) {
      ...
      let Temp__FollowingUser = [];

      for (const Following of FollowingUser) {
        Temp__FollowingUser.push(Following);
      }
      Temp__FollowingUser.push(All__User__Arr[i]);

      FBW.Update__User__FollowInfo__ById(Ovner__User.Id, [], Temp__FollowingUser);
    }
